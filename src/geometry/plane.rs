...
impl Plane {
    pub fn new(h_segments: u16, v_segments: u16) -> Self {
        Plane {
            width: 2.0,
            height: 2.0,
    ...
    pub fn new_by_pixel(width: f32, height: f32, h_segments: u16, v_segments: u16) -> Self {
        Plane {
            width,
            height,
            h_segments,
    ...
    pub fn generate_vertices(&self) -> (Vec<PosTex>, Vec<u16>) {
    ...
    // 返回的是线断列表，而不是线段条带
    pub fn get_line_indices(&self) -> Vec<u16> {
        ...
        for h in 0 ..= self.h_segments {
            ...
            let num: u16 = v_point_num * h;
            for v in 0 ..= self.v_segments {
                let current: u16 = num + v;
                // 找到上一列同一行位置的索引
                let left: u16 = current - v_point_num;
                if v == 0 {
                    indices.push(left);
                    indices.push(current);
                } else {
    ...
    pub fn get_element_indices(&self) -> Vec<u16> {
        let mut indices: Vec<u16> = Vec::new();
        // 2 个线段有 3 个结点，所以需要 (self.v_segments + 1) * h
        let v_point_num = self.v_segments + 1;
        // 按列遍历
